...
|--------|----------|---------|------------|
| Top |r| | 0.863 (afib) | 0.853 (hypothyroid) | 0.574 (V5867) |
...
**Key finding**: Custom SAEs produce correlations nearly 2x stronger than GemmaScope at the top end (0.86 vs 0.57). The gap widens dramatically at higher thresholds: at r>0.3, custom SAEs have 12-14x more grounded latents than GemmaScope.
...
### 1.6 TF-IDF + Logistic Regression Baseline (Vanilla SAE only)

| Metric | TF-IDF | SAE | Winner |
|--------|--------|-----|--------|
| Mean AUC-ROC | 0.917 | 0.881 | TF-IDF (33/46 codes) |
| Mean AUC-PR | 0.590 | 0.532 | TF-IDF (33/46 codes) |
| Best-feature r (supp.) | 0.519 | 0.579 | SAE (23/46 codes) |

**Critical nuance**: TF-IDF wins on classification AUC because it has 10,000 features (full vocabulary) vs SAE's unsupervised single-feature correlation. This is expected and not damaging: the SAE's best individual feature often produces a stronger univariate correlation than TF-IDF's best individual feature (23/46 codes). The SAE compresses 50k-token documents into 18k sparse features; it shouldn't beat a supervised classifier on per-code AUC.

**Gap**: TF-IDF baseline was never run for JumpReLU. This is a minor gap — the JumpReLU and Vanilla SAEs have nearly identical grounding profiles, so the results would likely be similar.
...
| r>0.4 | 280 | 85 | 180 | 7 | 8 | 94.6% | 30.4% |
...
|--------|----------------|-----------------|----------------|----------------|-------------------|-------------------|
| Top |r| | 0.864 | 0.863 | 0.860 | 0.853 | 0.545 | 0.574 |
...
**Key finding**: Test-split grounding is virtually identical to full-corpus grounding for the custom SAEs. The top correlations actually increase slightly on the test split (0.864 vs 0.863 for JumpReLU). This eliminates the overfitting concern: the SAE features' clinical associations generalize to unseen data.
...
**Chanin et al. (ICLR 2025) — "Sparse Autoencoders Do Not Find Canonical Units of Analysis"**
...
- **Impact on this paper**: This is the most dangerous paper for the narrative. However, it evaluates SAEs on *general-domain* interpretability metrics. This project's ICD grounding provides an *external ground truth* that random baselines could not match. The point-biserial correlations (r=0.86) against structured diagnosis labels are a much harder bar than interpretability scoring. This actually strengthens the case for domain-specific grounding as a validation methodology.
...
**Paulo et al. / Delphi (Eleuther AI, 2024)**: Established the fuzzing + detection scoring paradigm used in this project. Open-source, widely adopted.
...
**S1: External ground-truth validation.** The ICD-9 grounding is the project's most defensible contribution. Unlike most SAE interpretability work that relies on human judgment or LLM-based scoring (both subjective), this project validates features against structured diagnosis labels. In the context of Korznikov et al.'s critique that random baselines match SAEs on interpretability metrics, having point-biserial correlations of r=0.86 against external labels is a much harder test. Random feature directions would not produce r=0.86 against specific ICD codes.
...
**S7: Test-split validation.** Grounding results hold identically on held-out shards never used for SAE training (top |r| 0.864 test vs 0.863 full for JumpReLU). This eliminates overfitting concerns and is stronger than most SAE papers, which don't validate on held-out data.
...
**W4: Scorer ceiling effect undermines one validation modality.** Fuzzing and detection scores are essentially uninformative (0.93+ across all tiers including dead). The shuffled-explanation control is designed but not yet implemented. Without it, scorer accuracy cannot be reported as a validation metric.
...
**W6: TF-IDF baseline only for Vanilla.** Missing for JumpReLU (the primary model).

**W7: Small ablation sample.** 20 features is a limited sample for causal claims. The pilot had only 12 (10 grounded + 2 controls). Statistical power for per-feature significance is adequate given 4,911 notes, but the feature selection may not be representative.

**W8: No ICD-10 or SNOMED grounding.** Using ICD-9 codes (a 40-year-old coding system) when MIMIC-IV supports ICD-10 may raise reviewer questions about clinical relevance. (Counter: MIMIC-IV's discharge summaries historically use ICD-9; this is a data constraint.)

### 3.3 Gaps That Could Be Addressed Before Submission

| Gap | Effort | Impact |
|-----|--------|--------|
| Run shuffled-explanation control | ~2 hrs impl + 15 min Modal, ~$1 | High — rescues scorer validation |
...
A: The ceiling reflects the SAE's monospecificity — virtually all firing features encode coherent patterns. The concordance gradient (85% -> 99%) is independent of scorer accuracy and provides the discriminative validation that scorer accuracy cannot. [Pending: shuffled-explanation control to demonstrate explanation specificity.]
...
2. **Related Work**: Position against O'Neill (domain-specific SAEs, no external grounding), Sainsbury (coded sequences, not free text), Korznikov/Chanin (SAE critique papers — this work offers a partial response through external validation).
...
Lead with the **concordance gradient**, not the grounding correlations. The grounding correlation (r=0.86) is a strong result but could be seen as "just correlation." The concordance gradient — showing that unsupervised LLM explanations increasingly align with structured labels as the statistical association strengthens — is the methodological contribution that makes this paper publishable rather than just a nice empirical observation.
...
**As-is (without filling gaps)**: Borderline accept at EMNLP. The concordance gradient is novel and compelling, but reviewers will flag the missing JumpReLU ablation, scorer ceiling without shuffled control, and single model/layer limitation. Likely gets mixed reviews: one enthusiastic, one skeptical about SAE validity citing Korznikov, one asking for more models.

**With gaps filled (shuffled control + JumpReLU ablation + JumpReLU TF-IDF)**: Solid accept at EMNLP. The additional experiments don't change the story but they close reviewer objections. The shuffled control is particularly important because it converts the scorer ceiling from a weakness into a data point.
...
1. **Korznikov-inspired rejection**: A reviewer who takes "SAEs don't beat random baselines" as gospel may reject regardless of external validation. Mitigation: address this directly in related work, emphasize that external labels are a fundamentally different evaluation.
...
The concordance gradient is publishable. The question is whether the surrounding experiments are sufficient to pass reviewer scrutiny at a top venue. With the shuffled control and JumpReLU ablation, I believe they are.

**Estimated probability of acceptance at EMNLP 2026**:
- As-is: 35-45%
- With shuffled control + JumpReLU ablation: 55-65%
- With above + Haiku comparison + JumpReLU TF-IDF: 65-75%

---

## 7. Recommended Next Steps (Priority Order)

1. **Run shuffled-explanation control** (~2 hrs impl + 15 min Modal, ~$1) — highest ROI fix; rescues scorer accuracy as a publishable metric
2. **Run raw activation LR baseline** (~4-8 hrs on Modal) — tests whether SAE decomposition outperforms raw representations on classification; code already exists, just needs a Modal run
3. **Write paper draft** — structure per §5.3 above; all core experiments are complete

Items 1-2 can run in parallel on Modal.
